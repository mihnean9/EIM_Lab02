...
public class LifecycleMonitorActivity extends AppCompatActivity {
    ...
    protected void onRestoreInstanceState(Bundle inState) {
        ...
        if (inState.containsKey(Constants.USERNAME_EDIT_TEXT)) {
            EditText usernameEditText = (EditText)findViewById(R.id.username_edit_text);
            usernameEditText.setText(inState.getString(Constants.USERNAME_EDIT_TEXT));
        }
        if (inState.containsKey(Constants.PASSWORD_EDIT_TEXT)) {
            EditText passwordEditText = (EditText)findViewById(R.id.password_edit_text);
            passwordEditText.setText(inState.getString(Constants.PASSWORD_EDIT_TEXT));
        }
        if (inState.containsKey(Constants.REMEMBER_ME_CHECKBOX)) {
            CheckBox rememberMeCheckBox = (CheckBox) findViewById(R.id.remember_me_checkbox);
            rememberMeCheckBox.setChecked(inState.getBoolean(Constants.REMEMBER_ME_CHECKBOX));
        }
    }
}
